...
run = "preview_" + datetime.datetime.utcnow().strftime("%Y%m%d_%H%M%S") + "_vret2"
...
echo TRIM_VIDEOS
ffmpeg -y -v error -t 4 -i "preview_runs/$RUN/inputs/videos/094.mp4" -an -c:v libx264 -preset veryfast -crf 20 "preview_runs/$RUN/inputs/videos/094_4s.mp4"
ffmpeg -y -v error -t 4 -i "preview_runs/$RUN/inputs/videos/005.mp4" -an -c:v libx264 -preset veryfast -crf 20 "preview_runs/$RUN/inputs/videos/005_4s.mp4"
