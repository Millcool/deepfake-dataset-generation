...
paths = [
    "video-retalking/third_part/GPEN/face_model/op/fused_act.py",
    "video-retalking/third_part/GPEN/face_model/op/upfirdn2d.py",
]

new_contents = {
    "video-retalking/third_part/GPEN/face_model/op/fused_act.py": """import os
import platform
...
""",
    "video-retalking/third_part/GPEN/face_model/op/upfirdn2d.py": """import os
import platform
...
for path in paths:
    # Fetch current file (metadata only; we avoid content=1 for large files; these are tiny though).
    meta_url = f"{base}/api/contents/{path}?content=1"
    r = s.get(meta_url, headers=headers, timeout=60)
    r.raise_for_status()
    cur = r.json()
    cur_text = cur.get("content", "")
    fmt = cur.get("format") or "text"

    backup_path = f"{path}.bak_{ts}"
    put_url = f"{base}/api/contents/{backup_path}"
    rb = s.put(
        put_url,
        headers=headers,
        json={"type": "file", "format": fmt, "content": cur_text},
        timeout=120,
    )
    rb.raise_for_status()
    ...
    ru = s.put(
        ...
        headers=headers,
        json={"type": "file", "format": "text", "content": new_text},
        timeout=120,
    )
    ru.raise_for_status()
    print("updated", path, "bytes", len(new_text))
